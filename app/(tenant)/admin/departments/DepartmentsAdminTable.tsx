...
const ICON_ACTION =
  "flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-[color-mix(in_srgb,var(--outline-variant)_55%,transparent)] bg-[var(--surface-container-lowest)] text-[var(--on-surface-variant)] calm-transition hover:border-[color-mix(in_srgb,var(--outline-variant)_45%,transparent)] hover:bg-[var(--surface-container-low)] hover:text-[var(--on-surface)]";
            ...
            <div key={dept.id} className={DEPT_CARD}>
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-4 px-5 py-5 sm:px-6">
                <button
                  ...
                  className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md border border-[color-mix(in_srgb,var(--outline-variant)_55%,transparent)] bg-[var(--surface-container-lowest)] text-[var(--on-surface-variant)] calm-transition hover:border-[color-mix(in_srgb,var(--outline-variant)_45%,transparent)] hover:bg-[var(--surface-container-low)] hover:text-[var(--on-surface)]"
                ...
                <div className="min-w-0 flex-1 basis-[min(100%,14rem)]">
                  <p className="text-base font-bold tracking-tight text-[var(--on-surface)]">{dept.name}</p>
                  <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5">
                    <span className="inline-flex items-center gap-1.5 text-[0.8125rem] text-[var(--on-surface-variant)]">
                      <PersonGlyph className="h-4 w-4 shrink-0 opacity-80" />
                      <span className="tabular-nums">{staffCount}</span>
                      <span>staff</span>
                    </span>
                    {dept.faculty ? (
                      <span className="text-[0.75rem] text-[var(--on-surface-variant)]">{dept.faculty}</span>
                    ) : null}
                  </div>
                </div>
                ...
                <div className="flex w-full flex-wrap items-center justify-between gap-3 sm:ml-auto sm:w-auto sm:justify-end">
                  <div className="flex min-w-0 max-w-full items-center gap-3">
                    {hod ? (
                      ...
                      <div className="text-[0.8125rem] italic text-[var(--on-surface-variant)]">No head assigned</div>
                    )}
                  </div>
              ...
              {isExpanded ? (
                <div className="border-t border-[color-mix(in_srgb,var(--outline-variant)_55%,transparent)] px-5 pb-6 pt-5 sm:px-6">
                  <p className="text-[0.625rem] font-bold uppercase tracking-[0.1em] text-[var(--on-surface-variant)]">Head of department</p>
                  <div className="mt-3 rounded-xl border border-[color-mix(in_srgb,var(--outline-variant)_55%,transparent)] bg-[var(--surface-container-lowest)] px-4 py-3.5">
                    {hod ? (
                      ...
                      <p className="text-[0.8125rem] italic text-[var(--on-surface-variant)]">No head of department assigned yet.</p>
                    )}
                  </div>

                  <p className="mt-8 text-[0.625rem] font-bold uppercase tracking-[0.1em] text-[var(--on-surface-variant)]">
                    Staff in {dept.name}
                  </p>

                  <div className="mt-3 overflow-x-auto rounded-xl border border-[color-mix(in_srgb,var(--outline-variant)_55%,transparent)] bg-[var(--surface-container-lowest)]">
                    <table className="w-full min-w-[320px] border-collapse text-left">
                      <thead>
                        <tr className="border-b border-[color-mix(in_srgb,var(--outline-variant)_55%,transparent)] bg-transparent">
                          <th className="px-4 py-3 text-[0.625rem] font-bold uppercase tracking-[0.1em] text-[var(--on-surface-variant)]">Staff member</th>
                          <th className="px-4 py-3 text-[0.625rem] font-bold uppercase tracking-[0.1em] text-[var(--on-surface-variant)]">Role</th>
                          <th className="w-[1%] whitespace-nowrap px-4 py-3 text-right text-[0.625rem] font-bold uppercase tracking-[0.1em] text-[var(--on-surface-variant)]">
                      ...
                      <tbody>
                        {dept.memberships.length === 0 && addingMemberDeptId !== dept.id ? (
                          <tr>
                            <td colSpan={3} className="px-4 py-6 text-center text-[0.8125rem] italic text-[var(--on-surface-variant)]">
                        ...
                        {dept.memberships.map((m) => (
                          <tr key={m.userId} className="border-b border-[color-mix(in_srgb,var(--outline-variant)_55%,transparent)] last:border-b-0">
                            ...
                            <td className="px-4 py-3.5 text-[0.8125rem] text-[var(--on-surface-variant)]">
                  ...
                  {addingMemberDeptId === dept.id && availableToAdd.length > 0 ? (
                    <form
                      ...
                      className="mt-4 flex flex-wrap items-center gap-3 rounded-xl border border-[color-mix(in_srgb,var(--outline-variant)_55%,transparent)] bg-[var(--surface-container-low)] px-4 py-3"
                    >
                      <input type="hidden" name="departmentId" value={dept.id} />
                      <PersonGlyph className="h-5 w-5 shrink-0 text-[var(--on-surface-variant)]" />
                      <select
                        name="userId"
                        value={selectedUserId}
                        onChange={(e) => setSelectedUserId(e.target.value)}
                        className="min-h-[2.5rem] min-w-[12rem] flex-1 rounded-xl border border-[color-mix(in_srgb,var(--outline-variant)_55%,transparent)] bg-[var(--surface-container-lowest)] px-3 text-sm text-[var(--on-surface)] outline-none transition focus:ring-2 focus:ring-[rgba(15,23,42,0.06)]"
                        required
                      >
                      ...
                      <button
                        type="submit"
                        className="rounded-xl bg-[var(--on-surface)] px-4 py-2 text-[0.8125rem] font-semibold text-white shadow-sm calm-transition hover:bg-[var(--primary-container)]"
                      >
                        Add
                      </button>
                      <button
                        ...
                        className="rounded-xl border border-[color-mix(in_srgb,var(--outline-variant)_55%,transparent)] bg-[var(--surface-container-lowest)] px-4 py-2 text-[0.8125rem] font-semibold text-[var(--on-surface-variant)] calm-transition hover:bg-[var(--surface-container-low)] hover:text-[var(--on-surface)]"
                      >
                        Cancel
                      </button>
                    </form>
                  ...
                  {addingMemberDeptId === dept.id && availableToAdd.length === 0 ? (
                    <p className="mt-4 text-[0.8125rem] italic text-[var(--on-surface-variant)]">All staff are already members of this department.</p>
                  ) : null}
                </div>
              ) : null}
            </div>
      ...
      {editingDept ? (
        ...
        >
          <div className="w-full max-w-sm rounded-sm border border-border bg-[var(--surface-container-lowest)] p-6 shadow-none" role="dialog" aria-modal="true" aria-labelledby="dept-rename-title">
            <h2 id="dept-rename-title" className="mb-4 text-lg font-bold text-[var(--on-surface)]">
              Rename department
            </h2>
            <form action={renameDepartmentAction} onSubmit={() => setEditingDept(null)}>
              <input type="hidden" name="id" value={editingDept.id} />
              <label className="mb-1.5 block text-[0.6875rem] font-semibold uppercase tracking-[0.08em] text-[var(--on-surface-variant)]">Department name</label>
              <input name="name" value={editName} onChange={(e) => setEditName(e.target.value)} className="w-full rounded-xl border border-[color-mix(in_srgb,var(--outline-variant)_55%,transparent)] bg-[var(--surface-container-lowest)] px-3 py-2.5 text-sm text-[var(--on-surface)] outline-none transition focus:ring-2 focus:ring-[rgba(15,23,42,0.06)]" required />
              <div className="mt-5 flex justify-end gap-2">
                ...
                <button
                  type="submit"
                  className="rounded-xl bg-[var(--on-surface)] px-4 py-2 text-[0.8125rem] font-semibold text-white calm-transition hover:bg-[var(--primary-container)]"
                >
                  Save
                </button>
              </div>
