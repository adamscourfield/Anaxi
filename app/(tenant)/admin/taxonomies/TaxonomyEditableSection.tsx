...
export function TaxonomyEditableSection({
  ...
  return (
    <div className={CARD}>
      <div className="flex flex-col gap-3 border-b border-border/20 px-5 py-5 sm:flex-row sm:items-start sm:justify-between sm:px-7 sm:py-6">
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-[0.6875rem] font-semibold text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-300">
            ...
            <table className="w-full min-w-[640px] border-collapse text-left">
              <thead>
                <tr className="border-b border-border/25 bg-transparent">
                  ...
                  <th className="min-w-[240px] px-4 py-3 text-right text-[0.625rem] font-bold uppercase tracking-[0.1em] text-muted">Actions</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((row) => {
                  ...
                  return (
                    <tr key={row.id} className="border-b border-border/20 last:border-b-0" onDragEnter={() => handleDragEnter(row.id)}>
                      <td className="align-middle px-2 py-3">
                        <button
                          ...
                          className="flex h-9 w-9 cursor-grab items-center justify-center rounded-lg text-muted hover:bg-surface-container-low active:cursor-grabbing"
                      ...
                      <td className="min-w-0 px-4 py-3 align-middle">
                        <form id={formId} action={updateItem} className="flex min-w-0 items-center gap-2">
                          ...
                          <span className="h-2 w-2 shrink-0 rounded-full bg-muted/50" aria-hidden />
                          {field === "email" ? <EnvelopeIcon className="h-4 w-4 shrink-0 text-muted" /> : null}
        ...
        <div className="mt-6 rounded-xl bg-background p-5 sm:p-6">
          ...
          <form action={addItem} className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end">
            ...
            <label className="min-w-0 flex-1">
              <span className="mb-1.5 block text-[0.8125rem] font-semibold text-text">
            ...
            <button
              type="submit"
              className="inline-flex shrink-0 items-center justify-center rounded-xl bg-neutral-950 px-6 py-2.5 text-sm font-semibold text-white shadow-sm calm-transition hover:bg-neutral-900 dark:bg-white dark:text-neutral-950 dark:hover:bg-neutral-100"
            >
              Add
            </button>
          </form>
