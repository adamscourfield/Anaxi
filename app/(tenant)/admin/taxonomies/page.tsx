      ...
      <Link
        ...
        className={`flex items-start gap-3 rounded-xl border px-4 py-3.5 calm-transition ${
          active
            ...
            : "border-transparent hover:bg-[color-mix(in_srgb,var(--surface-container-low)_80%,transparent)]"
        }`}
      >
        <span
          ...
          <div className={AUTH_CARD}>
            ...
            <div className="space-y-2 px-5 pb-6 pt-4 sm:px-7">
              {(loaAuthorisers as any[]).map((row) => (
                <div
                  key={row.id}
                  className="flex flex-col gap-3 rounded-xl border border-border/25 bg-[color-mix(in_srgb,var(--surface-container-low)_50%,transparent)] px-4 py-3.5 sm:flex-row sm:items-center sm:justify-between"
                >
            ...
            <div className="mx-5 mb-6 rounded-xl bg-[color-mix(in_srgb,var(--surface-container-low)_65%,transparent)] p-5 sm:mx-7">
          ...
          <div className={AUTH_CARD}>
            ...
            <div className="space-y-4 px-5 pb-6 pt-4 sm:px-7">
              {Array.from(scopesByApprover.entries()).map(([approverId, { approver, targets }]) => (
                <div key={approverId} className="overflow-hidden rounded-xl border border-border/25 bg-[color-mix(in_srgb,var(--surface-container-low)_45%,transparent)]">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {targets.map((scope: any) => (
                        <form key={scope.id} action={removeScopedTarget} className="contents">
                          ...
                          <button
                            type="submit"
                            className="group inline-flex max-w-full items-center gap-1.5 rounded-full border border-border/30 bg-surface-container-lowest px-3 py-1.5 text-xs font-medium text-text shadow-sm calm-transition hover:border-error/30 hover:bg-error/5 hover:text-error"
                          >
